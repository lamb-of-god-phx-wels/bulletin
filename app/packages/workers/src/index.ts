/**
 * @cbb/workers — Electron utilityProcess workers for the Church Bulletin Builder.
 *
 * Each worker runs in a sandboxed OS process with no network access.
 * Workers handle untrusted content parsing: ZIP, SVG, raster decode,
 * font tables, PDF flattening, booklet composition, and PDF/UA validation.
 */

export const WORKERS_PACKAGE_NAME = "@cbb/workers" as const;

export * from "./quarantine/protocol.js";
export * from "./quarantine/broker.js";
export * from "./quarantine/nodeHandles.js";
export * from "./quarantine/nodeBubblewrapWorker.js";
export * from "./quarantine/nodePdfFlattenWorker.js";
export * from "./quarantine/routedWorker.js";
export * from "./execution/linuxResourceBroker.js";
export * from "./execution/pdfRuntimeClosure.js";
export * from "./windows/broker.js";
export * from "./windows/quarantine.js";
