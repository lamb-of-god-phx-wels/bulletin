...
export interface SvgHardLimits {
  readonly inputBytes: number;
  readonly xmlNodes: number;
  readonly pathCommands: number;
}

export interface RasterHardLimits {
  readonly inputBytes: number;
...
export interface FontHardLimits {
  readonly inputBytes: number;
}

export interface PdfHardLimits {
  readonly inputBytes: number;
...
export const QUARANTINE_HARD_LIMITS: QuarantineHardLimitsByOperation = Object.freeze({
  ...
  inspectFont: Object.freeze({ inputBytes: 50 * 1_048_576 }),
...
export interface FontSuccess extends SuccessBase<"inspectFont"> {
  ...
  readonly observed: FontHardLimits;
...
export interface PdfSuccess extends SuccessBase<"flattenPdf"> {
  ...
  readonly observed: PdfHardLimits;
...
const MEDIA_TYPES: Readonly<Record<QuarantineOperation, readonly QuarantineMediaType[]>> =
  Object.freeze({
    inspectArchive: ["application/zip"] as const,
  ...
  switch (operation) {
    ...
    case "sanitizeSvg": {
      const limits = exactRecord(
        value,
        ["inputBytes", "xmlNodes", "pathCommands"],
        "SVG limits",
      );
    ...
    case "canonicalizeRaster": {
      const limits = exactRecord(
        value,
        ["inputBytes", "decodedPixels", "width", "height"],
        "raster limits",
      );
    ...
    case "inspectFont": {
      const limits = exactRecord(value, ["inputBytes"], "font limits");
      bounded(
        limits["inputBytes"],
    ...
    case "flattenPdf": {
      const limits = exactRecord(value, ["inputBytes", "pages"], "PDF limits");
...
function validateArchiveResult(
  ...
  for (const value of entries) {
    const entry = exactRecord(
      value,
      ["path", "compressedBytes", "uncompressedBytes", "hash"],
      "archive entry",
    );
    if (!safeArchivePath(entry["path"]) || typeof entry["hash"] !== "string" || !HASH.test(entry["hash"])) {
    ...
    if (
      uncompressed > 0 &&
      (compressed === 0 ||
        BigInt(uncompressed) > BigInt(compressed) * BigInt(request.limits.compressionRatio))
  ...
  if (
    ...
    compressedTotal > request.limits.compressedBytes ||
    uncompressedTotal > request.limits.uncompressedBytes ||
    (uncompressedTotal > 0 &&
      (compressedTotal === 0 ||
        BigInt(uncompressedTotal) >
          BigInt(compressedTotal) * BigInt(request.limits.compressionRatio)))
  ) throw new RangeError("Archive aggregate observations are inconsistent or over limit");
}
...
function validateSuccess(result: Record<string, unknown>, request: QuarantineRequest): void {
  ...
  if (operation === "inspectArchive") {
    ...
    const observed = result["observed"] as unknown as RasterHardLimits;
    ...
    const observed = result["observed"] as unknown as PdfHardLimits;
...
/** Hard output bound supplied to the privileged opaque-handle verifier. */
