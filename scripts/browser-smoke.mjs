...
if (process.env.BULLETIN_FONT_PICKER_ONLY === '1') {
  ...
  await wait(`Boolean(document.querySelector('.canvas-designer'))`, 'page template canvas designer');
  ...
  const boundText = await evaluate(`document.querySelector('.canvas-stage.is-text-editing [contenteditable="true"]')?.textContent`);
  await evaluate(`(()=>{const editor=document.querySelector('.canvas-stage.is-text-editing [contenteditable="true"]'),range=document.createRange(),selection=getSelection();range.selectNodeContents(editor);selection.removeAllRanges();selection.addRange(range);document.dispatchEvent(new Event('selectionchange'));return true})()`);
  await command('Input.insertText', { text: 'Temporary override' });
  await wait(`document.querySelector('.canvas-stage.is-text-editing [contenteditable="true"]')?.textContent==='Temporary override'`, 'canvas bound text override');
  await evaluate(`(()=>{const editor=document.querySelector('.canvas-stage.is-text-editing [contenteditable="true"]'),range=document.createRange(),selection=getSelection();range.selectNodeContents(editor);selection.removeAllRanges();selection.addRange(range);document.dispatchEvent(new Event('selectionchange'));return true})()`);
  await evaluate(`document.querySelector('.canvas-rich-text-toolbar .font-picker-control')?.click()`);
  await wait(`document.querySelector('.font-picker-popover')?.matches(':popover-open')`, 'canvas reset font menu');
  await evaluate(`Array.from(document.querySelectorAll('.font-picker-popover [role="option"]')).find(button=>button.textContent.includes('Eras'))?.click()`);
  await wait(`Boolean(document.querySelector('.canvas-stage.is-text-editing [data-text-style*="bundled-eras"]'))`, 'formatted canvas bound override');
  await wait(`Boolean(Array.from(document.querySelectorAll('.canvas-properties button')).find(button=>button.textContent.trim()==='Reset to bound value'))`, 'canvas bound-value reset');
  await evaluate(`Array.from(document.querySelectorAll('.canvas-properties button')).find(button=>button.textContent.trim()==='Reset to bound value')?.click()`);
  await wait(`document.querySelector('.canvas-stage.is-text-editing [contenteditable="true"]')?.textContent===${JSON.stringify(boundText)}`, 'restored canvas bound text');
  if (!await evaluate(`Boolean(document.querySelector('.canvas-stage.is-text-editing [data-text-style*="bundled-eras"]'))`)) throw new Error('Resetting canvas bound text discarded its formatting.');
  pass('resets only canvas bound text while preserving its formatting');
...
if (process.env.BULLETIN_BUILDER_TODOS_ONLY === '1') {
  ...
  const originalTitle = await evaluate(`Array.from(document.querySelectorAll('label')).find(label=>label.textContent.trim().startsWith('Sermon title'))?.querySelector('input')?.value`);
  await fill('Sermon title', `${originalTitle} A`);
  await fill('Sermon title', `${originalTitle} AB`);
  await evaluate(`window.dispatchEvent(new KeyboardEvent('keydown',{key:'z',ctrlKey:true,bubbles:true,cancelable:true}))`);
  await wait(`Array.from(document.querySelectorAll('label')).find(label=>label.textContent.trim().startsWith('Sermon title'))?.querySelector('input')?.value===${JSON.stringify(`${originalTitle} A`)}`, 'atomic text undo');
  await evaluate(`window.dispatchEvent(new KeyboardEvent('keydown',{key:'y',ctrlKey:true,bubbles:true,cancelable:true}))`);
  await wait(`Array.from(document.querySelectorAll('label')).find(label=>label.textContent.trim().startsWith('Sermon title'))?.querySelector('input')?.value===${JSON.stringify(`${originalTitle} AB`)}`, 'atomic text redo');
  const navigationLabels = await evaluate(`Array.from(document.querySelectorAll('.sidebar nav button')).map(button=>button.textContent.trim())`);
  ...
  for (const label of ['Grid', 'Table']) if (!paletteLabels.includes(label)) throw new Error(`${label} container is missing from the palette.`);
  if (paletteLabels.includes('Stack')) throw new Error('The retired Stack container remains in the palette.');
  await click('Grid');
  await wait(`Boolean(document.querySelector('.preview-pane .block-group.layout-grid'))`, 'grid container preview');
  await evaluate(`Array.from(document.querySelectorAll('.block-editor')).find(item=>item.querySelector('.block-type')?.textContent.startsWith('group'))?.querySelector('.drag-handle')?.focus()`);
  ...
  await wait(`!document.querySelector('.preview-pane .block-group.layout-grid')`, 'structural undo from focused drag handle');
  await evaluate(`document.querySelector('.block-editor .drag-handle')?.focus()`);
  ...
  await wait(`Boolean(document.querySelector('.preview-pane .block-group.layout-grid'))`, 'structural redo');
  ...
  await wait(`Boolean(document.querySelector('.block-formatting-modal .format-actual-preview .block-group.layout-grid'))`, 'actual container format preview');
  ...
  const formatSize = await evaluate(`Number(Array.from(document.querySelectorAll('.block-formatting-modal label')).find(label=>label.textContent.trim().startsWith('Size (pt)'))?.querySelector('input')?.value)`);
  await fill('Size (pt)', String(formatSize + 1));
  await fill('Size (pt)', String(formatSize + 2));
  await evaluate(`window.dispatchEvent(new KeyboardEvent('keydown',{key:'z',ctrlKey:true,bubbles:true,cancelable:true}))`);
  await wait(`Number(Array.from(document.querySelectorAll('.block-formatting-modal label')).find(label=>label.textContent.trim().startsWith('Size (pt)'))?.querySelector('input')?.value)===${formatSize + 1}`, 'format-dialog atomic undo');
...
if (process.env.BULLETIN_PAGE_TEMPLATE_CANVAS_ONLY === '1') {
  ...
  await fill('Page template name', 'Browser canvas regression');
  await click('Canvas');
  await wait(`Boolean(document.querySelector('.canvas-designer'))`, 'new page canvas designer');
  const pageEditor = await evaluate(`(()=>{const sidebar=document.querySelector('.canvas-page-elements'),setup=sidebar?.querySelector('.page-setup-card'),properties=sidebar?.querySelector('.custom-properties-panel'),palette=sidebar?.querySelector(':scope > .element-palette');return {canvasLabel:document.querySelector('.canvas-designer .eyebrow')?.textContent,setup:Boolean(setup),properties:Boolean(properties),setupAbove:Boolean(setup&&palette&&setup.getBoundingClientRect().top<palette.getBoundingClientRect().top),design:Array.from(document.querySelectorAll('.canvas-designer button')).some(button=>button.textContent.trim()==='Design'),intermediate:Boolean(document.querySelector('.page-template-designer'))}})()`);
  if (!pageEditor.canvasLabel?.includes('canvas') || !pageEditor.setup || !pageEditor.properties || !pageEditor.setupAbove || pageEditor.design || pageEditor.intermediate) throw new Error(`Canvas page editor did not open directly: ${JSON.stringify(pageEditor)}`);
  const canvasPalette = await evaluate(`(()=>{const sidebar=document.querySelector('.canvas-elements-sidebar'),layers=document.querySelector('.canvas-layers'),workarea=document.querySelector('.canvas-workarea'),palette=sidebar?.querySelector(':scope > .element-palette'),item=palette?.querySelector('.element-palette-item'),scroll=palette?.querySelector('.element-palette-scroll'),style=palette&&getComputedStyle(palette),a=sidebar?.getBoundingClientRect(),b=layers?.getBoundingClientRect(),c=workarea?.getBoundingClientRect();return {docked:palette?.classList.contains('docked'),background:style?.backgroundColor,color:style?.color,itemHeight:item?.getBoundingClientRect().height,sidebarWidth:a?.width,nestedScroll:scroll&&scroll.scrollHeight>scroll.clientHeight+1,separateColumns:a&&b&&c&&Math.abs(a.right-b.left)<1&&Math.abs(b.right-c.left)<1}})()`);
  if (!canvasPalette.docked || canvasPalette.background !== 'rgb(32, 42, 38)' || canvasPalette.itemHeight < 41 || canvasPalette.itemHeight > 43 || canvasPalette.sidebarWidth < 299 || !canvasPalette.separateColumns) throw new Error(`Canvas Elements palette is not compact and correctly docked: ${JSON.stringify(canvasPalette)}`);
  const canvasViewport = await evaluate(`({zoom:Boolean(document.querySelector('.canvas-designer select[aria-label="Preview zoom"]')),rulers:document.querySelectorAll('.canvas-stage-frame .page-rulers').length,horizontalTicks:document.querySelectorAll('.canvas-stage-frame .ruler-horizontal .ruler-tick').length,verticalTicks:document.querySelectorAll('.canvas-stage-frame .ruler-vertical .ruler-tick').length,legacyLabels:document.querySelectorAll('.canvas-ruler-label').length,boundsSelector:Boolean(Array.from(document.querySelectorAll('.canvas-properties label')).find(label=>label.textContent.includes('Coordinate space'))),snap:document.querySelector('.canvas-designer button[aria-label$="snapping"]')?.getAttribute('aria-pressed'),guides:document.querySelector('.canvas-designer button[aria-label$="guides"]')?.getAttribute('aria-pressed'),boxes:document.querySelector('.canvas-designer button[aria-label$="content bounding boxes"]')?.getAttribute('aria-pressed')})`);
  if (!canvasViewport.zoom || canvasViewport.rulers !== 1 || canvasViewport.horizontalTicks !== 29 || canvasViewport.verticalTicks !== 35 || canvasViewport.legacyLabels || canvasViewport.boundsSelector || canvasViewport.snap !== 'true') throw new Error(`Canvas viewport controls are incomplete: ${JSON.stringify(canvasViewport)}`);
  await evaluate(`(()=>{const button=document.querySelector('.canvas-designer button[aria-label$="guides"]');if(button?.getAttribute('aria-pressed')!=='true')button?.click();return true})()`);
  await wait(`Boolean(document.querySelector('.canvas-stage .canvas-safe-guide'))&&localStorage.getItem('bulletin-show-guides')==='true'`, 'canvas guide toggle');
  await evaluate(`(()=>{const button=document.querySelector('.canvas-designer button[aria-label$="content bounding boxes"]');if(button?.getAttribute('aria-pressed')!=='true')button?.click();return true})()`);
  await wait(`document.querySelector('.canvas-selection-layer')?.classList.contains('show-bounds')&&localStorage.getItem('bulletin-canvas-show-bounds')==='true'`, 'canvas content bounding boxes toggle');
  ...
  const cornerHandles = await evaluate(`Array.from(document.querySelectorAll('.canvas-selection.selected .canvas-resize-handle')).map(handle=>handle.dataset.resizeCorner).sort()`);
  if (JSON.stringify(cornerHandles) !== JSON.stringify(['ne', 'nw', 'se', 'sw'])) throw new Error(`Canvas corner handles are incomplete: ${JSON.stringify(cornerHandles)}`);
  pass('shows all four canvas resize handles');
  const livePreviewBefore = await evaluate(`(()=>{const element=document.querySelector('.canvas-native-block'),preview=element?.querySelector('.preview-block'),rect=element?.getBoundingClientRect(),style=preview&&getComputedStyle(preview);return {height:rect?.height,width:rect?.width,fontSize:style?.fontSize,lineHeight:style?.lineHeight,textAlign:style?.textAlign}})()`);
  await evaluate(`document.querySelector('.canvas-selection.selected')?.dispatchEvent(new MouseEvent('dblclick',{bubbles:true,cancelable:true}))`);
  await wait(`Boolean(document.querySelector('.canvas-stage.is-text-editing [contenteditable="true"]'))`, 'stable canvas live text editor');
  const livePreviewAfter = await evaluate(`(()=>{const element=document.querySelector('.canvas-native-block'),preview=element?.querySelector('.preview-block'),rect=element?.getBoundingClientRect(),style=preview&&getComputedStyle(preview);return {height:rect?.height,width:rect?.width,fontSize:style?.fontSize,lineHeight:style?.lineHeight,textAlign:style?.textAlign}})()`);
  if (Math.abs(livePreviewBefore.height - livePreviewAfter.height) > .6 || Math.abs(livePreviewBefore.width - livePreviewAfter.width) > .6 || livePreviewBefore.fontSize !== livePreviewAfter.fontSize || livePreviewBefore.lineHeight !== livePreviewAfter.lineHeight || livePreviewBefore.textAlign !== livePreviewAfter.textAlign) throw new Error(`Canvas text preview changed while entering edit mode: ${JSON.stringify(livePreviewBefore)} → ${JSON.stringify(livePreviewAfter)}`);
  await command('Input.insertText', { text: 'x' });
  await new Promise(resolve => setTimeout(resolve, 100));
  const livePreviewTyped = await evaluate(`(()=>{const rect=document.querySelector('.canvas-native-block')?.getBoundingClientRect();return {height:rect?.height,width:rect?.width}})()`);
  if (livePreviewTyped.height + .6 < livePreviewAfter.height || Math.abs(livePreviewTyped.width - livePreviewAfter.width) > .6) throw new Error(`Canvas text box shrank while typing: ${JSON.stringify(livePreviewAfter)} → ${JSON.stringify(livePreviewTyped)}`);
  pass('keeps canvas text metrics and typography stable while editing');
  await command('Input.dispatchKeyEvent', { type: 'keyDown', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 });
  await command('Input.dispatchKeyEvent', { type: 'keyUp', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 });
  await wait(`!document.querySelector('.canvas-stage.is-text-editing')`, 'leave stable canvas text editor');
  ...
  const alignmentMiddle = await evaluate(`(()=>{const box=document.querySelector('.canvas-native-block').getBoundingClientRect(),content=document.querySelector('.canvas-native-content > .preview-block').getBoundingClientRect();return content.top-box.top})()`);
  ...
  const alignmentBottom = await evaluate(`(()=>{const box=document.querySelector('.canvas-native-block').getBoundingClientRect(),content=document.querySelector('.canvas-native-content > .preview-block').getBoundingClientRect();return content.top-box.top})()`);
  if (!(alignmentTop + 1 < alignmentMiddle && alignmentMiddle + 1 < alignmentBottom)) throw new Error(`Canvas vertical alignment did not move through the box: ${JSON.stringify({ alignmentTop, alignmentMiddle, alignmentBottom })}`);
  pass('positions canvas text correctly at the top, middle, and bottom');
  await command('Input.dispatchKeyEvent', { type: 'keyDown', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 });
  await command('Input.dispatchKeyEvent', { type: 'keyUp', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 });
  await wait(`!document.querySelector('.canvas-stage.is-text-editing')`, 'leave canvas text editing');
  const alignmentAfterExit = await evaluate(`(()=>{const box=document.querySelector('.canvas-native-block').getBoundingClientRect(),content=document.querySelector('.canvas-native-content > .preview-block').getBoundingClientRect();return content.top-box.top})()`);
  if (Math.abs(alignmentAfterExit - alignmentBottom) > .6) throw new Error(`Canvas vertical alignment changed after editing: ${alignmentBottom} → ${alignmentAfterExit}`);
  pass('preserves canvas vertical alignment after leaving text editing');
  ...
  await command('Input.dispatchKeyEvent', { type: 'keyDown', key: 'v', code: 'KeyV', windowsVirtualKeyCode: 86, modifiers: 2 });
  await command('Input.dispatchKeyEvent', { type: 'keyUp', key: 'v', code: 'KeyV', windowsVirtualKeyCode: 86, modifiers: 2 });
  await wait(`document.querySelectorAll('.canvas-stage [data-canvas-element-id]').length===${beforePaste + 1}`, 'canvas keyboard copy and paste');
  await command('Input.dispatchKeyEvent', { type: 'keyDown', key: 'Insert', code: 'Insert', windowsVirtualKeyCode: 45, modifiers: 2 });
  await command('Input.dispatchKeyEvent', { type: 'keyUp', key: 'Insert', code: 'Insert', windowsVirtualKeyCode: 45, modifiers: 2 });
  await command('Input.dispatchKeyEvent', { type: 'keyDown', key: 'Insert', code: 'Insert', windowsVirtualKeyCode: 45, modifiers: 8 });
  await command('Input.dispatchKeyEvent', { type: 'keyUp', key: 'Insert', code: 'Insert', windowsVirtualKeyCode: 45, modifiers: 8 });
  await wait(`document.querySelectorAll('.canvas-stage [data-canvas-element-id]').length===${beforePaste + 2}`, 'canvas Insert-key copy and paste');
  ...
  await wait(`document.querySelectorAll('.canvas-stage [data-canvas-element-id]').length===${beforePaste + 1}`, 'remove latest pasted canvas copy');
  for (const count of [beforePaste + 2, beforePaste + 1, beforePaste]) {
    await evaluate(`Array.from(document.querySelectorAll('.canvas-tools button')).find(button=>button.textContent.trim()==='Undo')?.click()`);
    await wait(`document.querySelectorAll('.canvas-stage [data-canvas-element-id]').length===${count}`, `clipboard undo ${count}`);
  }
  await evaluate(`Array.from(document.querySelectorAll('.canvas-layers li')).find(row=>row.querySelector('.canvas-layer-copy b')?.textContent==='Heading')?.querySelector('.canvas-layer-select')?.click()`);
  await wait(`Boolean(document.querySelector('.canvas-properties .canvas-format-button'))`, 'reselect canvas heading');
  ...
  await evaluate(`(()=>{const heading=Array.from(document.querySelectorAll('.canvas-layers li')).find(row=>row.querySelector('.canvas-layer-copy b')?.textContent==='Heading')?.querySelector('.canvas-layer-select');heading?.dispatchEvent(new MouseEvent('click',{bubbles:true,ctrlKey:true}));return Boolean(heading)})()`);
  await wait(`document.querySelectorAll('.canvas-selection.selected').length===2`, 'canvas Ctrl-click multi-selection');
  await evaluate(`Array.from(document.querySelectorAll('.canvas-tools button')).find(button=>button.textContent.trim()==='Group')?.click()`);
  await wait(`Array.from(document.querySelectorAll('.canvas-layers .canvas-layer-copy small')).filter(element=>element.textContent.includes('Grouped')).length===2`, 'canvas grouping');
  await evaluate(`Array.from(document.querySelectorAll('.canvas-tools button')).find(button=>button.textContent.trim()==='Ungroup')?.click()`);
  await wait(`!Array.from(document.querySelectorAll('.canvas-layers .canvas-layer-copy small')).some(element=>element.textContent.includes('Grouped'))`, 'canvas ungrouping');
  await evaluate(`window.dispatchEvent(new KeyboardEvent('keydown',{key:'a',ctrlKey:true,bubbles:true}))`);
  await wait(`document.querySelectorAll('.canvas-selection.selected').length===document.querySelectorAll('.canvas-selection').length`, 'canvas Ctrl+A select all');
  await evaluate(`(()=>{const heading=Array.from(document.querySelectorAll('.canvas-layers li')).find(row=>row.querySelector('.canvas-layer-copy b')?.textContent==='Heading');heading?.dispatchEvent(new MouseEvent('contextmenu',{bubbles:true,clientX:700,clientY:350}));return Boolean(heading)})()`);
  await wait(`Boolean(document.querySelector('.canvas-context-menu'))`, 'canvas layer context menu');
  await evaluate(`Array.from(document.querySelectorAll('.canvas-context-menu button')).find(button=>button.textContent.trim()==='Bring to front')?.click()`);
  await wait(`!document.querySelector('.canvas-context-menu')`, 'canvas layer context command');
  const layerDrag = await evaluate(`(()=>{const rows=Array.from(document.querySelectorAll('.canvas-layers ol li')),heading=rows.find(row=>row.querySelector('.canvas-layer-copy b')?.textContent==='Heading'),back=rows.at(-1),handle=heading?.querySelector('.drag-handle'),a=handle?.getBoundingClientRect(),b=back?.getBoundingClientRect();return {start:{x:a?.left+a?.width/2,y:a?.top+a?.height/2},end:{x:b?.left+b?.width/2,y:b?.top+b?.height/2}}})()`);
  ...
  await wait(`document.querySelector('.canvas-layers ol li:last-child .canvas-layer-copy b')?.textContent==='Heading'`, 'canvas layer drag reorder');
  await new Promise(resolve => setTimeout(resolve, 120));
  await evaluate(`Array.from(document.querySelectorAll('.canvas-designer-actions button')).find(button=>button.textContent.includes('Save draft'))?.click()`);
  await wait(`document.querySelector('.template-save-status')?.textContent.includes('Draft saved')`, 'browser canvas page save');
  await evaluate(`Array.from(document.querySelectorAll('.canvas-designer-actions button')).find(button=>button.textContent.trim()==='Done')?.click()`);
  await wait(`!document.querySelector('.canvas-designer')`, 'page template editor close');
  await evaluate(`(()=>{const record=Array.from(document.querySelectorAll('.library-record-entries [data-library-key]')).find(button=>button.textContent.includes('Browser canvas regression'));record?.click();return Boolean(record)})()`);
  await evaluate(`Array.from(document.querySelectorAll('.library-browser footer button')).find(button=>button.textContent.trim()==='Open')?.click()`);
  ...
  if (!await evaluate(`document.querySelector('.canvas-layers ol li:last-child .canvas-layer-copy b')?.textContent==='Heading'`)) throw new Error('Canvas layer drag order was not persisted.');
  const publishDisabled = await evaluate(`Array.from(document.querySelectorAll('.canvas-designer-actions button')).find(button=>button.textContent.includes('Publish version'))?.disabled`);
  if (publishDisabled) throw new Error('A valid browser canvas page cannot be published.');
  await evaluate(`Array.from(document.querySelectorAll('.canvas-designer-actions button')).find(button=>button.textContent.includes('Publish version'))?.click()`);
...
if (process.env.BULLETIN_PAGE_TEMPLATE_REGULAR_ONLY === '1') {
  ...
  await fill('Page template name', 'Browser regular regression');
  await click('Regular layout');
  await wait(`Boolean(document.querySelector('.page-template-designer.page-template-regular'))`, 'new regular page template editor');
  const regularLayout = await evaluate(`(()=>{const sidebar=document.querySelector('.page-template-elements'),editor=document.querySelector('.page-template-flow-editor'),preview=document.querySelector('.page-template-preview'),setup=sidebar?.querySelector('.page-setup-card'),palette=sidebar?.querySelector('.element-palette'),page=preview?.querySelector('.document-page'),content=page?.querySelector(':scope > .page-content');const a=sidebar?.getBoundingClientRect(),b=editor?.getBoundingClientRect(),c=preview?.getBoundingClientRect();return {dark:getComputedStyle(sidebar).backgroundColor,columns:Boolean(a&&b&&c&&Math.abs(a.right-b.left)<1&&Math.abs(b.right-c.left)<1),setup:Boolean(setup),setupAbove:Boolean(setup&&palette&&setup.getBoundingClientRect().top<palette.getBoundingClientRect().top),palette:Boolean(palette),blank:Boolean(page&&content&&!content.children.length),missing:preview?.textContent.includes('Missing'),zoom:Boolean(preview?.querySelector('select[aria-label="Preview zoom"]'))}})()`);
  if (regularLayout.dark !== 'rgb(39, 50, 46)' || !regularLayout.columns || !regularLayout.setup || !regularLayout.setupAbove || !regularLayout.palette || !regularLayout.blank || regularLayout.missing || !regularLayout.zoom) throw new Error(`Regular page template editor does not match the bulletin layout: ${JSON.stringify(regularLayout)}`);
  await evaluate(`(()=>{const palette=document.querySelector('.page-template-elements .element-palette');if(palette?.classList.contains('collapsed'))palette.querySelector('header button')?.click();return true})()`);
  ...
  await command('Input.dispatchMouseEvent', { type: 'mousePressed', x: regularDrag.start.x, y: regularDrag.start.y, button: 'left', buttons: 1, clickCount: 1 });
  for (let step = 1; step <= 6; step++) {
    const ratio = step / 6;
    await command('Input.dispatchMouseEvent', { type: 'mouseMoved', x: regularDrag.start.x, y: regularDrag.start.y + (regularDrag.end.y - regularDrag.start.y) * ratio, button: 'left', buttons: 1 });
    await new Promise(resolve => setTimeout(resolve, 35));
  }
  await command('Input.dispatchMouseEvent', { type: 'mouseReleased', x: regularDrag.end.x, y: regularDrag.end.y, button: 'left', buttons: 0, clickCount: 1 });
  await wait(`document.querySelector('.page-template-flow-editor .block-editor')?.dataset.editorBlockId!==${JSON.stringify(regularDrag.first)}`, 'regular page block reorder');
  pass('uses the bulletin editor layout for regular page templates, including a blank page, docked palette, formatting, and sorting');
  if (runtimeErrors.length) throw new Error(`Runtime errors: ${runtimeErrors.join('\\n')}`);
  console.log(`\n${results.length} browser regular-page checks passed.`);
  socket.close();
  process.exit(0);
}

if (process.env.BULLETIN_PALETTE_ONLY === '1') {
  const expandedShell = await evaluate(`(()=>{const slot=document.querySelector('.sidebar-palette-slot')?.getBoundingClientRect(),palette=document.querySelector('.elements-sidebar .element-palette')?.getBoundingClientRect(),drawer=document.querySelector('.navigation-drawer')?.getBoundingClientRect();return {editor:document.querySelector('.app-shell')?.classList.contains('editor-shell'),open:document.querySelector('.app-shell')?.classList.contains('navigation-open'),drawer:drawer?{left:drawer.left,width:drawer.width}:null,rail:Boolean(document.querySelector('.navigation-rail')),elements:Boolean(palette),collapse:Boolean(document.querySelector('.elements-sidebar .element-palette > header button')),bottomGap:slot&&palette?slot.bottom-palette.bottom:null}})()`);
  if (!expandedShell.editor || !expandedShell.open || !expandedShell.drawer || expandedShell.rail || !expandedShell.elements || expandedShell.collapse || Math.abs(expandedShell.bottomGap ?? 99) > 1 || Math.abs(expandedShell.drawer.left) > 1 || Math.abs(expandedShell.drawer.width - 232) > 1) throw new Error(`Expanded editor shell is incomplete: ${JSON.stringify(expandedShell)}`);
  pass('shows the full navigation and dedicated non-collapsible Elements sidebar by default');

  const beforeCollapse = await evaluate(`(()=>{const main=document.querySelector('.main-area').getBoundingClientRect(),elements=document.querySelector('.elements-sidebar').getBoundingClientRect();return {main:{left:main.left,width:main.width},elements:{left:elements.left,width:elements.width}}})()`);
  await evaluate(`document.querySelector('.navigation-close')?.click()`);
  await wait(`!document.querySelector('.app-shell')?.classList.contains('navigation-open')`, 'navigation collapse button');
  await new Promise(resolve => setTimeout(resolve, 220));
  const afterCollapse = await evaluate(`(()=>{const main=document.querySelector('.main-area').getBoundingClientRect(),elements=document.querySelector('.elements-sidebar').getBoundingClientRect(),rail=document.querySelector('.navigation-rail')?.getBoundingClientRect();return {main:{left:main.left,width:main.width},elements:{left:elements.left,width:elements.width},rail:rail?{left:rail.left,width:rail.width}:null}})()`);
  if (!afterCollapse.rail || Math.abs(afterCollapse.rail.left) > 1 || Math.abs(afterCollapse.rail.width - 56) > 1 || Math.abs(beforeCollapse.main.left - afterCollapse.main.left - 176) > 1 || Math.abs(afterCollapse.main.width - beforeCollapse.main.width - 176) > 1 || Math.abs(beforeCollapse.elements.left - afterCollapse.elements.left - 176) > 1 || Math.abs(beforeCollapse.elements.width - afterCollapse.elements.width) > 1) {
    throw new Error(`Navigation did not push the editor shell: ${JSON.stringify({ beforeCollapse, afterCollapse })}`);
  }
  pass('collapses the full navigation to a 56px rail and gives its space back to the workspace');
  ...
  pass('collapses navigation by its header control and Escape');
  ...
  await wait(`Boolean(document.querySelector('.canvas-designer'))`, 'canvas page override designer');
...
if (process.env.BULLETIN_CANVAS_ONLY === '1') {
  ...
  const resizeHandles = await queryAll('.canvas-designer .canvas-selection.selected .canvas-resize-handle');
  if (resizeHandles.length !== 4) throw new Error(`Canvas selection has ${resizeHandles.length} resize handles instead of four.`);
  pass('shows a resize handle on every canvas selection corner');
  const liveText = await evaluate(`(()=>{const element=Array.from(document.querySelectorAll('.canvas-designer .canvas-native-block')).find(item=>!document.querySelector('[data-canvas-selection-id="'+CSS.escape(item.dataset.canvasElementId)+'"]')?.classList.contains('locked'));if(!element)return null;const selection=document.querySelector('[data-canvas-selection-id="'+CSS.escape(element.dataset.canvasElementId)+'"]');const before=element.getBoundingClientRect();selection.dispatchEvent(new MouseEvent('dblclick',{bubbles:true,cancelable:true}));return {id:element.dataset.canvasElementId,height:before.height,width:before.width}})()`);
  if (!liveText) throw new Error('Canvas designer has no editable text object.');
  await wait(`document.querySelector('[data-canvas-selection-id="${liveText.id}"]')?.classList.contains('text-editing')`, 'canvas live text editor');
  const liveTextAfter = await evaluate(`(()=>{const element=document.querySelector('.canvas-designer .canvas-native-block[data-canvas-element-id="${liveText.id}"]');const rect=element.getBoundingClientRect();return {height:rect.height,width:rect.width}})()`);
  if (Math.abs(liveText.height - liveTextAfter.height) > .6 || Math.abs(liveText.width - liveTextAfter.width) > .6) throw new Error(`Canvas text changed size when editing: ${JSON.stringify(liveText)} → ${JSON.stringify(liveTextAfter)}`);
  pass('keeps canvas text box metrics stable while editing');
  await evaluate(`document.querySelector('.canvas-designer .canvas-stage')?.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true}))`);
  ...
  const doneNode = (await queryAll('.canvas-designer-actions > button:last-child'))[0];
