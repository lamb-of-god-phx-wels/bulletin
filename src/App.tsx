...
import { CreateFromDialog, type CreationSource } from './components/CreateFromDialog';
...
import { duplicateTemplate, nextTemplateVersion, sortedTemplateRecords, templateChoices, templateForReference, templateFromBulletin, templateVersions, type TemplateRecord } from './shared/templates';
...
import { duplicateBulletin, filterBulletins, sortedBulletins, type BulletinRecord } from './shared/bulletins';

type Screen = 'weekly' | 'templates' | 'library' | 'church-year' | 'archive';
type Confirmation = { title: string; message: string; confirmLabel: string; action(): Promise<void> };
type BulletinConflictState = { path: string; document: BulletinDocumentV1; message: string };
...
function DesktopApp() {
  ...
  const [createDestination, setCreateDestination] = useState<'bulletin' | 'template'>();
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      void window.bulletin!.saveBulletin(workspace.root, relativePath, document, expected).then(result => {
        ...
        if (/conflict/i.test(message)) setBulletinConflict({ path: relativePath, document, message });
  ...
  useEffect(() => {
    ...
    return window.bulletin.onWorkspaceChanged(change => {
      ...
      void window.bulletin!.openWorkspace(workspace.root).then(next => {
        setWorkspace(next);
        const nextDocument = next.bulletins.find(item => item.path === relativePath);
        if (nextDocument && nextDocument.document.revision !== savedRevision.current) {
          setDocument(nextDocument.document);
          savedRevision.current = nextDocument.document.revision;
        }
        const nextTemplate = next.templates.find(item => item.path === templatePath);
        ...
        reportStatus(conflicts ? `${conflicts} shared conflict${conflicts === 1 ? '' : 's'} need attention` : pending ? `${pending} asset${pending === 1 ? '' : 's'} waiting for synchronization` : 'Shared workspace refreshed');
  ...
  function openNewBulletin(next: BulletinDocumentV1) { const base = `bulletins/${next.info.date}/bulletin.json`; const path = workspace?.bulletins.some(item => item.path === base) ? `bulletins/${next.info.date}/bulletin-${Date.now()}.json` : base; openDocument(next, path); updateEditingState({ bulletinDirty: true }); }
  function startNew(from = template) { openNewBulletin(createBulletin(from)); }
  function beginNewBulletin() { setCreateDestination('bulletin'); }
  ...
  function confirmBulletinDelete() {
    if (!document) return;
    setConfirmation({ title: 'Archive bulletin?', message: `“${document.info.title}” for ${document.info.date} will move to the synchronized archive.`, confirmLabel: 'Archive bulletin', action: deleteCurrentBulletin });
  }
  ...
  async function createNewTemplate(source: CreationSource, name: string) {
    if (!workspace || !window.bulletin) return;
    const next = source.kind === 'template'
      ? duplicateTemplate(source.record.template, name, workspace.templates)
      : templateFromBulletin(source.record.document, templateForBulletin(templateForReference(workspace.templates, source.record.document.template)?.template ?? template, source.record.document), name, workspace.templates);
    try {
      ...
      setTemplate(next); setTemplatePath(path); setScreen('templates'); reportStatus(`Created ${name}`);
  ...
  async function deleteCurrentTemplateFamily() {
    ...
    try {
      ...
      reportStatus(`Archived ${template.name} and ${versions.length} version${versions.length === 1 ? '' : 's'}`);
    } catch (error) { reportStatus(error instanceof Error ? error.message : String(error)); }
  }
  function confirmTemplateVersionDelete() {
    setConfirmation({ title: 'Archive template version?', message: `${template.name} version ${template.version}${template.status === 'draft' ? ' draft' : ''} will move to the synchronized archive. Other versions will remain available.`, confirmLabel: 'Archive version', action: deleteCurrentTemplateVersion });
  }
  function confirmTemplateFamilyDelete() {
    const versions = workspace ? templateVersions(workspace.templates, template.id).length : 0;
    setConfirmation({ title: 'Archive template?', message: `“${template.name}” and all ${versions} version${versions === 1 ? '' : 's'} will move to the synchronized archive.`, confirmLabel: 'Archive template', action: deleteCurrentTemplateFamily });
  }
  ...
  return <div className={`app-shell${workspaceWritable ? '' : ' workspace-readonly'}`}>
    ...
    <main className="main-area">
      ...
      {screen === 'church-year' && <ChurchYearView library={workspace.library ?? { schemaVersion: 1, name: 'Church Library', items: [] }} onDirtyChange={value => updateEditingState({ auxiliaryDirty: value })} onSave={async library => { if (!window.bulletin) return; try { await window.bulletin.saveLibrary(workspace.root, library, workspace.library); setWorkspace({ ...workspace, library }); reportStatus('Church Year overrides saved'); } catch (error) { const message = error instanceof Error ? error.message : String(error); reportStatus(message); throw error; } }} />}
    ...
    {createDestination && <CreateFromDialog destination={createDestination} templates={sortedTemplateRecords(workspace.templates)} bulletins={workspace.bulletins} initialTemplatePath={templatePath} onCancel={() => setCreateDestination(undefined)} onCreate={async (source, value) => {
      if (createDestination === 'bulletin') {
        setCreateDestination(undefined);
        const next = source.kind === 'template' ? createBulletin(source.record.template, value) : duplicateBulletin(source.record.document, value);
...
function ConfirmDialog({ confirmation, onCancel, onConfirm }: { confirmation: Confirmation; onCancel(): void; onConfirm(): Promise<void> }) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onCancel(); }}><section className="confirmation-modal" role="alertdialog" aria-modal="true" aria-labelledby="confirmation-title"><div className="eyebrow">Please confirm</div><h2 id="confirmation-title">{confirmation.title}</h2><p>{confirmation.message}</p><p>This change will synchronize to other workspace users.</p><div><button className="secondary" autoFocus onClick={onCancel}>Cancel</button><button className="danger" onClick={() => void onConfirm()}>{confirmation.confirmLabel}</button></div></section></div>;
}
...
function LibraryView({ workspace, onSave, onError, onDirtyChange }: { workspace: WorkspaceSummary; onSave(library: LibraryManifestV1): Promise<void>; onError(message: string): void; onDirtyChange?(dirty: boolean): void }) {
  ...
  const requestDelete = (item: LibraryItemV1) => {
    ...
    setDeleteConfirmation({ title: 'Archive library item?', message: `${item.title}, version ${item.version}, will be hidden in the shared library and retained in the synchronized archive.${references ? ` It is currently referenced ${references} time${references === 1 ? '' : 's'}; existing references remain unchanged.` : ''}`, confirmLabel: 'Archive item', action: async () => onSave({ ...(workspace.library ?? { schemaVersion: 1, name: 'Church Library' }), items: items.filter(entry => entry.id !== item.id || entry.version !== item.version) }) });
  };
  ...
  return <div className="library-screen"><div className="library-intro"><div><div className="eyebrow">Approved content</div><h2>{families.length} library item{families.length === 1 ? '' : 's'}</h2><p>{items.length} saved version{items.length === 1 ? '' : 's'} of songs, liturgy, artwork, fonts, and licensing metadata in the synced workspace.</p></div><div><button className="primary" onClick={() => { if (adding) closeForm(); else { setEditing(undefined); setDraft(emptyLibraryDraft()); setAdding(true); } }}>{adding ? 'Close form' : '＋ Add library item'}</button><div className="library-path">{workspace.root}/library/</div></div></div>{adding && <section className="editor-card library-form"><h2>{editing ? `Edit ${editing.title}` : 'Add library item'}</h2>{editing && <p className="helper">Saving creates version {Math.max(...items.filter(item => item.id === editing.id).map(item => item.version)) + 1}; existing bulletins pinned to version {editing.version} will not change.</p>}<div className="field-row"><label>Title<input value={draft.title} onChange={e => setDraft({ ...draft, title: e.target.value, id: draft.id || e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, '-') })} /></label><label>Stable ID<input disabled={Boolean(editing)} value={draft.id} onChange={e => setDraft({ ...draft, id: e.target.value })} /></label></div><label>Kind<select value={draft.kind} onChange={e => setDraft({ ...draft, kind: e.target.value as LibraryItemV1['kind'] })}><option value="song">Song</option><option value="liturgy">Liturgy</option><option value="image">Image</option><option value="church-info">Church information</option><option value="font">Font</option></select></label><label>Structured text<textarea rows={6} value={draft.text} onChange={e => setDraft({ ...draft, text: e.target.value })} placeholder="Separate paragraphs or verses with a blank line" /></label><label>Copyright or license notice<textarea rows={3} value={draft.notice} onChange={e => setDraft({ ...draft, notice: e.target.value })} /></label><div className="builder-actions"><button className="secondary" disabled={!draft.id} onClick={chooseAsset}>{draft.asset ? `Replace ${draft.asset.alt ?? 'image or PDF'}` : 'Attach image or PDF'}</button><button className="secondary" onClick={closeForm}>Cancel</button><button className="primary" onClick={addItem}>{editing ? 'Save new version' : 'Save item'}</button></div></section>}{families.length === 0 ? <div className="empty-state"><span>▤</span><h2>Your shared library is ready</h2><p>Add the first reusable item above. Weekly editors will immediately offer songs and liturgy.</p></div> : Object.entries(groups).map(([kind, entries]) => <section className="library-group" key={kind}><h3>{kind}</h3>{entries?.map(family => { const item = selectedItem(family); return <article key={family.id}><div><b>{item.title}</b><small>{item.id} · {family.versions.length} version{family.versions.length === 1 ? '' : 's'}</small></div><div className="library-item-actions"><select className="inline-version-select" aria-label={`Version for ${family.id}`} value={item.version} onChange={event => setSelectedVersions(current => ({ ...current, [family.id]: Number(event.target.value) }))}>{family.versions.map(version => <option value={version.version} key={version.version}>v{version.version}{version.title !== family.versions[0].title ? ` · ${version.title}` : ''}</option>)}</select><span>{item.license ? 'Licensed' : 'No notice'}</span><button className="text-button" onClick={() => beginEdit(item)}>Edit</button><button className="danger-text" onClick={() => requestDelete(item)}>Archive</button></div></article>; })}</section>)}{deleteConfirmation && <ConfirmDialog confirmation={deleteConfirmation} onCancel={() => setDeleteConfirmation(undefined)} onConfirm={async () => { const action = deleteConfirmation.action; setDeleteConfirmation(undefined); await action(); }} />}</div>;
}

function ArchiveView({ records, onRestore, onDelete }: { records: ArchivedWorkspaceRecord[]; onRestore(record: ArchivedWorkspaceRecord): Promise<void>; onDelete(record: ArchivedWorkspaceRecord): Promise<void> }) {
  const [confirmation, setConfirmation] = useState<ArchivedWorkspaceRecord>();
  return <div className="library-screen"><div className="library-intro"><div><div className="eyebrow">Recoverable shared records</div><h2>{records.length} archived item{records.length === 1 ? '' : 's'}</h2><p>Archived items remain in SharePoint and can be restored on every connected PC.</p></div></div>
    {!records.length ? <div className="empty-state"><span>⌫</span><h2>The archive is empty</h2><p>Archived bulletins, templates, songs, overrides, and components will appear here.</p></div> : <section className="library-group"><h3>Archived records</h3>{records.map(record => <article key={record.id}><div><b>{record.label}</b><small>{record.kind} · {new Date(record.archivedAt).toLocaleString()}</small></div><div className="library-item-actions"><button className="text-button" onClick={() => void onRestore(record)}>Restore</button><button className="danger-text" onClick={() => setConfirmation(record)}>Delete permanently</button></div></article>)}</section>}
    {confirmation && <ConfirmDialog confirmation={{ title: 'Permanently delete archived item?', message: `“${confirmation.label}” content will be removed. A small synchronized tombstone will remain to prevent an offline copy from restoring it.`, confirmLabel: 'Delete permanently', action: () => onDelete(confirmation) }} onCancel={() => setConfirmation(undefined)} onConfirm={async () => { const record = confirmation; setConfirmation(undefined); await onDelete(record); }} />}
  </div>;
}

function SyncCenter({ conflicts, onClose, onKeep }: { conflicts: WorkspaceConflict[]; onClose(): void; onKeep(conflict: WorkspaceConflict, keepPath: string): Promise<void> }) {
  const [saving, setSaving] = useState(false);
  return <div className="modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="bulletin-picker-modal sync-center" role="dialog" aria-modal="true" aria-labelledby="sync-center-title"><header><div><div className="eyebrow">SharePoint synchronization</div><h2 id="sync-center-title">Resolve shared copies</h2><p>No copy is removed until you choose the one to retain.</p></div><button aria-label="Close synchronization center" onClick={onClose}>×</button></header>
    <div className="bulletin-picker-list">{conflicts.map(conflict => <article key={conflict.id}><div><b>{conflict.recordId}</b><small>{conflict.kind}</small><p>{conflict.message}</p></div><div>{conflict.paths.map(copy => <button disabled={saving} key={copy} onClick={async () => { setSaving(true); try { await onKeep(conflict, copy); } finally { setSaving(false); } }}><span><b>{copy}</b><small>Keep this synchronized copy and archive the others</small></span><strong>Keep</strong></button>)}</div></article>)}</div>
  </section></div>;
}

function BulletinConflictDialog({ conflict, onDefer, onUseShared, onKeepCopy, onReplaceShared }: {
  ...
  const [saving, setSaving] = useState(false);
  const act = async (action: () => void | Promise<void>) => { setSaving(true); try { await action(); } finally { setSaving(false); } };
  return <div className="modal-backdrop" role="presentation"><section className="confirmation-modal sync-conflict-dialog" role="alertdialog" aria-modal="true" aria-labelledby="bulletin-conflict-title"><div className="eyebrow">Concurrent bulletin edit</div><h2 id="bulletin-conflict-title">This bulletin changed on another PC</h2><p>{conflict.message}</p><p>Your unsaved version remains in this window until you choose what to do.</p><div className="conflict-actions"><button className="secondary" disabled={saving} onClick={onDefer}>Decide later</button><button className="secondary" disabled={saving} onClick={() => void act(onUseShared)}>Use shared version</button><button className="secondary" disabled={saving} onClick={onKeepCopy}>Keep mine as a copy</button><button className="danger" disabled={saving} onClick={() => void act(onReplaceShared)}>Replace shared version</button></div></section></div>;
}
...
function PrintApp() {
  ...
  const hasPdfCanvas = job.document.blocks.some(block => block.type === 'canvasCover' && (block.weeklyScene ?? block.scene).background?.asset?.mediaType === 'application/pdf');
  return <div className={`print-screen ${browser ? 'browser-print' : 'electron-print'}`}>{browser && <header className="print-controls"><div><b>{ready ? 'Print preview ready' : 'Preparing pages…'}</b><span>{hasPdfCanvas ? 'Browser printing rasterizes PDF cover backgrounds; desktop export retains the original PDF.' : 'Choose “Save to PDF” in your browser’s print dialog.'}</span></div><div className="print-actions"><button className="secondary" onClick={() => { if (history.length > 1) history.back(); else window.close(); }}>Back to editor</button><button className="primary" disabled={!ready} onClick={() => window.print()}>Print / Save as PDF</button></div></header>}<DocumentView document={job.document} template={template} library={workspace.library} root={job.root} print onReady={() => { if (browser) setReady(true); else window.bulletin?.printReady(); }} /></div>;
}
