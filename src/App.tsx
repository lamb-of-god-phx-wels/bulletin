...
function LibraryView({ workspace, onSave, onError }: { workspace: WorkspaceSummary; onSave(library: LibraryManifestV1): Promise<void>; onError(message: string): void }) {
  ...
  const addItem = async () => {
    ...
    try { await onSave({ ...(workspace.library ?? { schemaVersion: 1, name: 'Church Library' }), items: [...items, item] }); setAdding(false); setEditing(undefined); setDraft(emptyLibraryDraft()); }
