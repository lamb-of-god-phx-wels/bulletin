...
export function BlockFormattingModal({ block, template, document, library, assets = {}, scope, name, hidePageFlow = false, onClose, onSave }: { block: BulletinBlock; template: TemplateV1; document?: BulletinDocumentV1; library?: LibraryManifestV1; assets?: Record<string, string>; scope: 'template' | 'weekly'; name?: string; hidePageFlow?: boolean; onClose(): void; onSave(presentation: Partial<CustomBlockStyle> | undefined, layout: LayoutHints | undefined): void }) {
  ...
  return <div className="modal-backdrop block-modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="block-formatting-modal" role="dialog" aria-modal="true" aria-labelledby="format-block-title">
    ...
    <div className="formatting-body"><aside><div className="format-preview-page"><div className="format-actual-preview"><NativeBlockPreview block={previewBlock} library={library} assets={assets} document={previewDocument} template={template} marginIn={template.theme.marginIn} /></div></div><div className="format-scope-note"><b>{scope === 'weekly' ? 'Weekly override' : 'Template default'}</b><span>{scope === 'weekly' ? 'Only this bulletin JSON will be changed.' : 'Existing bulletins remain unchanged.'}</span></div></aside><main>
      {!hidePageFlow && <section className="appearance-section"><h3>Page flow</h3><div className="format-checks"><div className="toggle-option-row"><span>Start on a new page</span><ToggleSwitch label="Start on a new page" checked={layout.pageBreakBefore ?? false} onChange={pageBreakBefore => setLayout(current => ({ ...current, pageBreakBefore }))} /></div><div className="toggle-option-row"><span>Keep block together</span><ToggleSwitch label="Keep block together" checked={layout.keepTogether ?? false} onChange={keepTogether => setLayout(current => ({ ...current, keepTogether }))} /></div></div><label>Content spacing<select value={layout.density ?? 'normal'} onChange={event => setLayout(current => ({ ...current, density: event.target.value as 'normal' | 'compact' }))}><option value="normal">Comfortable</option><option value="compact">Compact</option></select></label></section>}
