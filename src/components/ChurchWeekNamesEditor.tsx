  ...
  return <details className="editor-card church-week-names">
    ...
    <div className="church-week-name-list">
      {names.map((name, index) => <div className="church-week-name-row" key={index}>
        ...
        <button className="danger-text" aria-label={`Remove church-week override ${index + 1}`} onClick={() => { setSaved(false); setNames(current => current.filter((_name, itemIndex) => itemIndex !== index)); }}>Remove</button>
      </div>)}
      {!names.length && <p className="helper">No overrides saved. Choosing a date will prompt when Service Builder returns a new name.</p>}
    </div>
    <div className="builder-actions"><button className="secondary" onClick={() => { setSaved(false); setNames(current => [...current, blankName()]); }}>＋ Add override</button><button className="primary" disabled={saving} onClick={() => void save()}>{saving ? 'Saving…' : saved ? 'Saved' : 'Save overrides'}</button></div>
  </details>;
}
