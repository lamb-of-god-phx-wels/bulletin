...
import { churchCalendarIssues, churchEventDates, churchEventsForDate, churchLectionaryYear, welsCalendarPreset } from '../shared/churchCalendar';
...
function RuleFields({ rule, events, onChange, onRemove }: { rule: ChurchEventRule; events: ChurchCalendarEvent[]; onChange(rule: ChurchEventRule): void; onRemove(): void }) {
  ...
  return <div className="calendar-rule">
    ...
    {(rule.kind === 'nthWeekday' || rule.kind === 'weekdayOnOrAfter' || rule.kind === 'weekdayRelative') && <label>Weekday<select value={rule.weekday} onChange={event => number('weekday', Number(event.target.value))}>{weekdays.map((name, index) => <option value={index} key={name}>{name}</option>)}</select></label>}
...
function EventEditor({ value, events, onChange, onClose, onDelete }: { value: ChurchCalendarEvent; events: ChurchCalendarEvent[]; onChange(value: ChurchCalendarEvent): void; onClose(): void; onDelete(): void }) {
  ...
  return <aside className="calendar-event-editor">
    ...
    <label>Name<input value={value.name} onChange={event => onChange({ ...value, name: event.target.value })} /></label>
    ...
    <div className="calendar-occurrence-preview"><b>Upcoming occurrences</b>{upcoming.length ? upcoming.map(date => <span key={date}>{new Date(`${date}T12:00:00`).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' })}</span>) : <span>No dates produced by these rules.</span>}</div>
    ...
    <main className="church-calendar-layout">
      <section className="church-calendar-grid">
        ...
        {cells.map((date, index) => date ? <button className={`calendar-day ${date === selectedDate ? 'selected' : ''} ${date === new Date().toISOString().slice(0, 10) ? 'today' : ''}`} key={date} onClick={() => setSelectedDate(date)}>
          ...
          <div>{churchEventsForDate(date, events).slice(0, 4).map(event => <i className={event.id === churchEventsForDate(date, events)[0]?.id ? 'default' : ''} key={event.id} onClick={click => { click.stopPropagation(); setSelectedDate(date); setEditing(structuredClone(event)); }}>{event.name}</i>)}</div>
      ...
      <aside className="calendar-day-panel">
        ...
        <div className="calendar-day-events">{selectedEvents.map((event, index) => <button key={event.id} onClick={() => setEditing(structuredClone(event))}><span>{event.name}</span><small>{index === 0 ? 'Bulletin default' : `Priority ${event.priority}`}</small></button>)}{!selectedEvents.length && <p>No church events occur on this date.</p>}</div>
