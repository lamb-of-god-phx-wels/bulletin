...
export function CreateFromDialog({ destination, templates, bulletins, initialTemplatePath, onCancel, onCreate }: {
  ...
  const [sourceKind, setSourceKind] = useState<CreationSource['kind'] | undefined>(destination === 'bulletin' ? 'template' : undefined);
  ...
  if (destination === 'template' && !sourceKind) return <div className="modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onCancel(); }}>
    <section className="create-from-modal create-kind-modal" role="dialog" aria-modal="true" aria-labelledby="create-from-title">
      <header><div><div className="eyebrow">New template</div><h2 id="create-from-title">How would you like to start?</h2><p>Choose the starting point for your new bulletin template.</p></div><button aria-label="Close" onClick={onCancel}>×</button></header>
      <div className="create-kind-options">
        <button onClick={() => switchKind('blank')}><span>□</span><b>Blank template</b><small>Start with an empty outline and the standard page and typography settings.</small></button>
        <button onClick={() => switchKind('template')}><span>◇</span><b>Existing template</b><small>Copy an existing bulletin template, including its elements and properties.</small></button>
        <button onClick={() => switchKind('bulletin')}><span>▦</span><b>Existing bulletin</b><small>Turn a completed or in-progress bulletin into a reusable template.</small></button>
      </div>
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onCancel(); }}>
    <section className="create-from-modal" role="dialog" aria-modal="true" aria-labelledby="create-from-title">
      <header><div><div className="eyebrow">New {destination}</div><h2 id="create-from-title">{title}</h2><p>{sourceKind === 'blank' ? 'Start with a clean, empty template.' : `Choose the existing ${sourceKind} to use as your starting point.`}</p></div><button aria-label="Close" onClick={onCancel}>×</button></header>
      ...
      <div className="create-source-list">
        {sourceKind === 'blank' && <div className="create-source-empty"><b>Empty starter outline</b><span>Add pages, text, songs, images, and other elements after creating the template.</span></div>}
      ...
      <footer><span>{selected ? `Using ${selected.kind === 'blank' ? 'a blank template' : selected.kind === 'template' ? selected.record.template.name : `${selected.record.document.info.title} · ${selected.record.document.info.date}`}` : 'Choose a source to continue.'}</span><div>{destination === 'template' && <button className="secondary" onClick={() => setSourceKind(undefined)}>Back</button>}<button className="secondary" onClick={onCancel}>Cancel</button><button className="primary" disabled={!selected || !value.trim() || creating} onClick={() => void create()}>{creating ? 'Creating…' : title}</button></div></footer>
    </section>
  </div>;
}
