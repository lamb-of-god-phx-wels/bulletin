...
function BlockView({ block, library, assets, document, template, marginIn, onBlockChange }: { block: PaginatedBlock; library?: LibraryManifestV1; assets: Record<string, string>; document: BulletinDocumentV1; template?: TemplateV1; marginIn: number; onBlockChange?(block: BulletinBlock): void }) {
  ...
  switch (block.type) {
    ...
    case 'list': {
      const items = block.items.map((item, itemIndex) => <li className={item.asset ? `announcement-with-asset asset-${item.assetSide ?? 'right'}` : undefined} key={item.id}>{item.asset && <FlowAsset asset={item.asset} source={assets[item.asset.path]} />}<div>{block.headingsEnabled !== false && (item.title || item.titleContent?.length) && <h3><EditableParagraphs content={item.titleContent ?? textParagraphs(item.title ?? '')} label="List item heading" onChange={onBlockChange ? next => onBlockChange({ ...block, items: block.items.map((candidate, index) => index === itemIndex ? { ...candidate, title: plainText(next), titleContent: next } : candidate) }) : undefined} /></h3>}<EditableParagraphs content={item.content} label="List item" onChange={onBlockChange ? next => onBlockChange(updateListItemContent(block, itemIndex, next).block) : undefined} /></div></li>);
