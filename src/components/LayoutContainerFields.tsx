...
export function LayoutContainerFields({ block, onChange, onAdd }: {
  ...
  return <>
    <div className="field-row container-options">
      {mode === 'table' ? <label>Layout<input value="Table" disabled /></label> : <label>Layout<select value={mode} onChange={event => onChange({ ...block, layoutMode: event.target.value as 'stack' | 'grid', gridSizing: 'equal', columnWidths: undefined, rowHeightsIn: undefined })}><option value="stack">Stack</option><option value="grid">Grid</option></select></label>}
