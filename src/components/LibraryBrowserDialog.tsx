...
export function LibraryBrowserDialog({
  ...
  const visibleFolders = searching || folderId === builtinsFolder ? [] : folders.filter(folder => folder.parentId === folderId).sort((a, b) => a.name.localeCompare(b.name));
  const visibleRecords = records.filter(record => {
    if (folderId === builtinsFolder ? !record.builtin : record.builtin) return false;
    const inFolder = searching
  ...
  const createFolder = async () => {
    ...
    if (!name?.trim()) return;
    if (!imageFolderNameAvailable(library, name, folderId)) return;
    await onLibraryChange({ ...library, folders: [...folders, { id: `library-folder-${randomId()}`, name: name.trim(), ...(folderId ? { parentId: folderId } : {}) }] });
  };
  const rename = async () => {
    ...
    if (key.startsWith('folder:')) {
      ...
      if (!name?.trim() || !imageFolderNameAvailable(library, name, folder.parentId, folder.id)) return;
      await onLibraryChange({ ...library, folders: folders.map(item => item.id === folder.id ? { ...item, name: name.trim() } : item) });
    } else {
      const record = records.find(item => item.key === key); if (!record || record.builtin) return;
      const name = window.prompt('Library display name', record.title);
      if (!name?.trim()) return;
      await onLibraryChange(setCatalogEntry(library, {
        targetKind: record.targetKind, targetId: record.targetId, folderId: record.folderId, displayName: name.trim()
      }));
    }
  };
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (event.key.toLocaleLowerCase() === 'v' && manage && cut.size && folderId !== builtinsFolder) { event.preventDefault(); void moveKeys(cut, folderId); }
    };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
  }, [selected, cut, folderId, library, records]);
  ...
  const content = <section className={`library-browser ${embedded ? 'embedded' : ''}`} role={embedded ? undefined : 'dialog'} aria-modal={embedded ? undefined : true}>
    ...
    <div className="library-browser-toolbar">
      {manage && <><button className="secondary" onClick={() => void createFolder()}>＋ Folder</button><label>New<select value="" onChange={event => { const type = event.target.value as LibraryRecordType; if (type) onCreate?.(type, folderId); }}><option value="">Choose type…</option>{types.map(type => <option value={type} key={type}>{libraryRecordTypeLabel[type]}</option>)}</select></label><button className="secondary" disabled={selected.size !== 1 || selectedRecord?.builtin} onClick={() => void rename()}>Rename</button><label>Move to<select value="" disabled={!selected.size} onChange={event => { if (event.target.value) void moveKeys(selected, event.target.value === '__root__' ? undefined : event.target.value); }}><option value="">Choose…</option><option value="__root__">Library root</option>{folders.map(folder => <option value={folder.id} key={folder.id}>{' '.repeat(imageFolderAncestors(library, folder.id).length * 2)}{folder.name}</option>)}</select></label><button className="danger-text" disabled={!selected.size || [...selected].some(key => records.find(record => record.key === key)?.builtin)} onClick={() => void onDelete?.(records.filter(record => selected.has(record.key)), [...selected].filter(key => key.startsWith('folder:')).map(key => key.slice(7)))}>Delete</button></>}
      {actions}<span /><button className={view === 'thumbnails' ? 'active' : ''} onClick={() => { setView('thumbnails'); localStorage.setItem(viewKey(root), 'thumbnails'); }}>▦</button><button className={view === 'list' ? 'active' : ''} onClick={() => { setView('list'); localStorage.setItem(viewKey(root), 'list'); }}>☷</button>
    </div>
    <div className="library-browser-body">
      ...
      <main onDragOver={event => { if (manage && folderId !== builtinsFolder) event.preventDefault(); }} onDrop={event => dropInto(folderId === builtinsFolder ? undefined : folderId, event)}>
        <div className="library-browser-path"><button onClick={() => openFolder(undefined)}>Library</button>{folderId === builtinsFolder ? <span>› Built-ins</span> : path.map(folder => <span key={folder.id}>› <button onClick={() => openFolder(folder.id)}>{folder.name}</button></span>)}<input type="search" placeholder="Search this folder tree" value={search} onChange={event => setSearch(event.target.value)} /></div>
