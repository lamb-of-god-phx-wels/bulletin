...
export function WeeklyEditor({ document, template, library, root, relativePath, onChange, onLibraryChange, onError, onAuxiliaryDirtyChange }: { document: BulletinDocumentV1; template: TemplateV1; library?: LibraryManifestV1; root?: string; relativePath: string; onChange(document: BulletinDocumentV1): void; onLibraryChange(library: LibraryManifestV1): Promise<void>; onError(message: string): void; onAuxiliaryDirtyChange?(dirty: boolean): void }) {
  ...
  return <div className="editor-scroll">
    ...
    <details className="editor-card collapsible-editor page-setup-card"><summary><div><span className="block-type">Document</span><h3>Page setup</h3></div><small>{document.layout?.marginIn ?? template.theme.marginIn} in margins</small></summary><div className="collapsible-editor-fields"><p className="helper">Physical page: 7 × 8.5 inches. The PDF print dialog should use no additional margins.</p><div className="page-margin-control"><label>Page margin (inches)<input type="number" min="0" max="1.25" step="0.05" value={document.layout?.marginIn ?? template.theme.marginIn} onChange={event => { if (Number.isFinite(event.currentTarget.valueAsNumber)) updatePageMargin(event.currentTarget.valueAsNumber); }} /><small className="field-help">Applies to this bulletin only. Template default: {template.theme.marginIn} in.</small></label><button type="button" className="text-button" disabled={document.layout?.marginIn === undefined} onClick={resetPageMargin}>Use template margin</button></div></div></details>
    <div className="editor-section-title"><div><div className="eyebrow">Order of worship</div><h2>Weekly content</h2><small>{document.blocks.length} blocks · changes apply only to this bulletin</small></div><div className="weekly-content-actions"><button className="primary" onClick={() => setBlockLibraryIndex(document.blocks.length)}>＋ Add block</button><button className="secondary" onClick={addPage}>＋ One-off page</button></div></div>
