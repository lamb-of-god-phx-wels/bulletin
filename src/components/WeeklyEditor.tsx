...
export function WeeklyEditor({ document, template, library, root, relativePath, onChange, onError }: { document: BulletinDocumentV1; template: TemplateV1; library?: LibraryManifestV1; root?: string; relativePath: string; onChange(document: BulletinDocumentV1): void; onError(message: string): void }) {
  ...
  const blockName = (block: BulletinBlock) => block.type === 'custom' ? block.name : block.label ?? ('text' in block ? block.text : block.type === 'announcements' ? 'Announcements' : block.type === 'titlePage' ? 'Cover' : block.type);
  ...
  return <div className="editor-scroll">
    ...
    <div className="editor-section-title"><div><div className="eyebrow">Order of worship</div><h2>Weekly content</h2></div><div className="weekly-content-actions"><button className="secondary" onClick={() => setFormatPickerOpen(true)}>Fine-tune layout</button><button className="secondary" onClick={addPage}>＋ One-off page</button></div></div>
    {document.blocks.map((block, index) => (block.weeklyEditable || missingLibraryReference(block)) && <section className="editor-card block-editor" key={block.id}>
      <header><div><span className="block-type">{block.type}{block.presentation ? ' · formatted' : ''}</span><h3>{blockName(block)}</h3></div><div className="reorder"><button className="format-block-button" title="Format block" onClick={() => setFormattingBlockId(block.id)}>Format</button><button title="Move up" onClick={() => move(index, -1)}>↑</button><button title="Move down" onClick={() => move(index, 1)}>↓</button></div></header>
      ...
      {block.type === 'custom' && <div className="custom-weekly-fields">{block.bindings.filter(binding => binding.source === 'weekly').map(binding => <label key={binding.key}>{binding.label}{binding.multiline
