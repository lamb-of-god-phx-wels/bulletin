...
export function WeeklyEditor({ document, library, root, relativePath, onChange }: { document: BulletinDocumentV1; library?: LibraryManifestV1; root?: string; relativePath: string; onChange(document: BulletinDocumentV1): void }) {
  const [expanded, setExpanded] = useState<string>();
  const [gateway, setGateway] = useState({ username: '', password: '' });
  ...
  const lookup = async (block: Extract<BulletinBlock, { type: 'scriptureReading' }>) => {
    ...
    setLookupStatus('Looking up passage…');
    try { const resolved = await window.bulletin.lookupScripture({ reference: block.reference, translation: block.translation, ...gateway }); updateBlock(block.id, { ...block, resolved }); setLookupStatus('Passage added'); }
    catch (error) { setLookupStatus(error instanceof Error ? error.message : String(error)); }
  };
  return <div className="editor-scroll">
    ...
    <details className="gateway-settings"><summary>Bible Gateway connection</summary><div className="field-row"><label>Username<input autoComplete="username" value={gateway.username} onChange={e => setGateway({ ...gateway, username: e.target.value })} /></label><label>Password<input type="password" autoComplete="current-password" value={gateway.password} onChange={e => setGateway({ ...gateway, password: e.target.value })} /></label></div><p className="helper">Credentials stay in this app session. Manual passage entry remains available below.</p></details>
    ...
    {document.blocks.map((block, index) => block.weeklyEditable && <section className="editor-card block-editor" key={block.id}>
      ...
      {block.type === 'libraryText' && <label>Library text<select value={block.libraryItemId} onChange={e => { const item = library?.items.filter(item => item.kind === 'liturgy').sort((a, b) => b.version - a.version).find(item => item.id === e.target.value); updateBlock(block.id, { ...block, libraryItemId: e.target.value, libraryItemVersion: item?.version }); }}><option value="">Choose reusable text…</option>{[...new Map(library?.items.filter(item => item.kind === 'liturgy').sort((a, b) => a.version - b.version).map(item => [item.id, item])).values()].map(item => <option value={item.id} key={item.id}>{item.title}</option>)}</select></label>}
