...
export function WeeklyEditor({ document, template, library, root, relativePath, onChange, onLibraryChange, onError }: { document: BulletinDocumentV1; template: TemplateV1; library?: LibraryManifestV1; root?: string; relativePath: string; onChange(document: BulletinDocumentV1): void; onLibraryChange(library: LibraryManifestV1): Promise<void>; onError(message: string): void }) {
  ...
  const nestedEditors = (parent: BulletinBlock): ReactNode => {
    ...
    return <div className="nested-blocks"><div className="nested-blocks-heading"><b>{isScripture ? 'Element layout' : isParagraph ? 'Text blocks' : 'Paragraphs'}</b><span>{isScripture ? 'Heading, reference, caption, and body can be positioned and formatted independently.' : isParagraph ? 'Header and body formatting are completely independent.' : 'Each paragraph keeps its header and body together.'}</span></div>{children.map((child, childIndex) => <details className="nested-block-editor collapsible-editor" data-editor-block-id={child.id} tabIndex={-1} key={child.id}><summary><div><span className="block-type">{child.type}{child.presentation ? ' · formatted' : ''}</span><b>{blockName(child)}</b></div><div className="reorder" onClick={event => event.preventDefault()}><button className="format-block-button" onClick={() => setFormattingBlockId(child.id)}>Format</button>{!isParagraph && !isScripture && <><button title="Move up" onClick={() => { const next = [...children]; if (childIndex > 0) [next[childIndex - 1], next[childIndex]] = [next[childIndex], next[childIndex - 1]]; updateChildren(parent, next); }}>↑</button><button title="Move down" onClick={() => { const next = [...children]; if (childIndex < next.length - 1) [next[childIndex + 1], next[childIndex]] = [next[childIndex], next[childIndex + 1]]; updateChildren(parent, next); }}>↓</button></>}{!isScripture && (!isParagraph || child.role === 'header') && <button className="danger-text" title="Remove element" onClick={() => updateChildren(parent, children.filter(item => item.id !== child.id))}>×</button>}</div></summary><div className="collapsible-editor-fields">{isScripture ? <p className="helper">Edit this element’s content above. Use Format for its width, placement, spacing, typography, fill, and border.</p> : <>{(child.type === 'heading' || child.type === 'sectionHeading' || child.type === 'sermonTitle') && <label>Heading<input value={child.text} onChange={event => updateBlock(child.id, { ...child, text: event.target.value })} /></label>}{child.type === 'richText' && <label>{child.role === 'header' ? 'Header text' : child.role === 'body' ? 'Paragraph text' : 'Text'}<textarea rows={child.role === 'header' ? 2 : 4} value={paragraphText(child.content)} onChange={event => updateBlock(child.id, { ...child, content: paragraphs(event.target.value) })} /></label>}{childBlocks(child) && nestedEditors(child)}</>}</div></details>)}{!isScripture && <div className="nested-add-actions">{isParagraph ? !children.some(child => child.type === 'richText' && child.role === 'header') && <button className="secondary" onClick={() => updateChildren(parent, [{ id: `${parent.id}-header`, type: 'richText', role: 'header', content: paragraphs('New heading'), presentation: { fontWeight: 'bold', marginIn: { top: 0, bottom: 0 }, paddingIn: { top: 0, right: 0, bottom: 0, left: 0 } } }, ...children])}>＋ Header</button> : <button className="secondary" onClick={() => updateChildren(parent, [...children, { id: `paragraph-${Date.now()}`, type: 'paragraph', children: [{ id: `paragraph-body-${Date.now()}`, type: 'richText', role: 'body', content: paragraphs('New text'), presentation: { marginIn: { top: 0, bottom: 0 }, paddingIn: { top: 0, right: 0, bottom: 0, left: 0 } } }] }])}>＋ Paragraph</button>}</div>}</div>;
  };
  const move = (index: number, by: number) => {
    const next = moveWeeklyBlock(document.blocks, index, by);
    if (next !== document.blocks) onChange({ ...document, blocks: next });
  };
  const addBlock = (descriptor: Parameters<typeof instantiateBlockDescriptor>[0]) => {
    const block = { ...instantiateBlockDescriptor(descriptor), weeklyEditable: true } as BulletinBlock;
  ...
  return <div className="editor-scroll">
    ...
    {document.blocks.map((block, index) => <details className="editor-card block-editor collapsible-editor" data-editor-block-id={block.id} tabIndex={-1} key={block.id}>
      ...
      {block.type === 'scriptureReading' && <><div className="field-row"><label>Heading (optional)<input value={block.label ?? ''} placeholder="First Reading" onChange={e => updateBlock(block.id, { ...block, label: e.target.value || undefined })} /></label><label>Scripture reference<input value={block.reference} placeholder="Matthew 9:9-13" onChange={e => updateBlock(block.id, { ...block, reference: e.target.value, resolved: undefined })} /></label></div><div className="field-row"><label>Caption (optional)<textarea rows={2} value={block.caption ?? ''} onChange={e => updateBlock(block.id, { ...block, caption: e.target.value || undefined })} /></label><label>Translation code<input value={block.translation} placeholder="NIV" onChange={e => updateBlock(block.id, { ...block, translation: e.target.value, resolved: undefined })} /></label></div><div className="scripture-actions"><button className="secondary scripture-fetch" disabled={lookupStatus[block.id]?.state === 'loading' || !block.reference.trim()} onClick={() => lookup(block)}>{lookupStatus[block.id]?.state === 'loading' ? 'Importing…' : 'Import passage'}</button><button className="text-button" onClick={() => openScripture(block)}>Open on Bible Gateway ↗</button></div>{lookupStatus[block.id] && <p className={`lookup-status ${lookupStatus[block.id].state}`} role="status" aria-live="polite">{lookupStatus[block.id].text}</p>}<details><summary>Body — passage text or manual fallback</summary><ScriptureEditor content={block.resolved?.content ?? [{ type: 'paragraph', children: [{ type: 'text', text: '' }] }]} onChange={content => updateBlock(block.id, { ...block, resolved: block.resolved ? { ...block.resolved, content } : { content, source: 'manual', retrievedAt: new Date().toISOString(), attribution: `${block.translation.toUpperCase()} — text supplied by user` } })} /></details>{nestedEditors(block)}</>}
    ...
    {blockLibraryIndex !== undefined && <BlockLibraryModal workspaceDescriptors={library?.blockDescriptors ?? []} template={template} library={library} root={root} onClose={() => setBlockLibraryIndex(undefined)} onUsePrepackaged={addBlock} onUseDescriptor={addBlock} onSaveDescriptor={async descriptor => onLibraryChange({ ...(library ?? { schemaVersion: 1, name: 'Shared Library', items: [] }), blockDescriptors: [...(library?.blockDescriptors ?? []), descriptor] })} onDeleteDescriptor={async descriptor => onLibraryChange({ ...(library ?? { schemaVersion: 1, name: 'Shared Library', items: [] }), blockDescriptors: (library?.blockDescriptors ?? []).filter(item => item.id !== descriptor.id || item.version !== descriptor.version) })} />}
