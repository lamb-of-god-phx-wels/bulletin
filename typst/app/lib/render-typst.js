...
#set page(width: ${page.typstWidth || "7in"}, height: ${page.typstHeight || "8.5in"}, margin: (left: ${typstLength(margins.left)}, right: ${typstLength(margins.right)}, top: ${typstLength(margins.top)}, bottom: ${typstLength(margins.bottom)}), fill: ${color(page.background || "#ffffff")})
#set text(font: "Calibri", size: 11pt)
#set par(first-line-indent: 0pt, spacing: 0.35em)

${elements.map((element) => renderElement(element, { assetPrefix, page, margins, content })).join("\n\n")}
`;
}
...
function renderElement(element, context) {
  ...
  return `${margin}#block(${boxArgs(element, null, context)})[
${indent(body, 4)}
]${margin ? `\n${margin.trimEnd()}` : ""}`;
}
...
function renderElementBox(element, context, forcedSize) {
  ...
  return `#box(${boxArgs(element, forcedSize, context)})[
...
function boxArgs(element, forcedSize, context) {
  const style = element.style || {};
  const args = [
    `width: ${typstWidth(forcedSize?.width ?? element.width, context)}`,
...
function canvasSize(element, context) {
  const pageWidth = context.content?.width || 672;
  const pageHeight = context.content?.height || 816;
  ...
  const width = Math.min(pageWidth, Math.max(requestedWidth, content.width));
...
function canvasContentExtent(element, baseWidth, context) {
  const baseHeight = context.content?.height || 816;
  return (element.children || []).reduce((extent, child) => {
    ...
    return {
      width: Math.max(extent.width, childX + size.width),
      height: Math.max(extent.height, childY + size.height),
    };
  }, { width: 0, height: 0 });
}
...
function elementSize(element, baseWidth, context) {
  ...
  return {
    ...
    height: Math.max(1, pixelLength(element.height, context.content?.height || 816, 80)),
  };
}
...
function pageContentBox(page) {
  const margins = pageMargins(page);
  const width = numericLength(page.width || 672);
  const height = numericLength(page.height || 816);
  return {
    width: Math.max(1, width - pixelLength(margins.left, width, 0) - pixelLength(margins.right, width, 0)),
    height: Math.max(1, height - pixelLength(margins.top, height, 0) - pixelLength(margins.bottom, height, 0)),
  };
}
...
function typstWidth(value, context) {
  const text = String(value ?? "").trim();
  if (text === "auto" || text.endsWith("%") || text.endsWith("fr")) return typstLength(value, { allowAuto: true });
  const maxWidth = context?.content?.width;
  if (!maxWidth) return typstLength(value, { allowAuto: true });
  return typstLength(Math.min(pixelLength(value, maxWidth, maxWidth), maxWidth));
}
...
function pixelLength(value, base, fallback = 0) {
  ...
  if (text.endsWith("in")) return Number.parseFloat(text) * 96;
  if (text.endsWith("cm")) return (Number.parseFloat(text) / 2.54) * 96;
  if (text.endsWith("mm")) return (Number.parseFloat(text) / 25.4) * 96;
